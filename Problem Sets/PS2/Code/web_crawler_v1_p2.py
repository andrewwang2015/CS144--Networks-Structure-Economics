...
def return_filtered_urls(url):
    ...
    new_links_in_domain = []
    for i in new_links:
        if 'access.caltech.edu' in i:
            new_links_in_domain.append(i)
    #new_links_in_domain = list(filter(lambda x: is_in_domain(x), new_links))
    #print(len(new_links_in_domain))
...
def main():
    ...
    max_num_links = 5  # Threshold for how many links to visit
    startingURL = "http://www.caltech.edu/"
    urlQueue =[]
    for i in range(10):
        if i % 2 == 0:
            urlQueue.append('https://access.caltech.edu/content/work')
        else:
            urlQueue.append('http://www.caltech.edu/')
    lst = []
    print(len(fetcher2.fetch_links('https://access.caltech.edu/content/work')))
    while (num_links_visited < max_num_links):
        nextURL = urlQueue.pop(0)
        visited_links.add(nextURL)
        all_domain_links = []
        print(len(fetcher2.fetch_links(nextURL)))
        # try:
        ...
        #update_dictionaries(nextURL, all_domain_links, adjacency, numFromPage, numToPage)
    ...
    print()
    #print(return_filtered_urls(startingURL))
    print(len(fetcher2.fetch_links('https://access.caltech.edu/content/work')))
    '''
